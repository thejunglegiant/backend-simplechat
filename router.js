...
router.get('/:userId/:roomId/getExactRoom', async (req, res) => {
    ...
    if (currentUser != null) {
        const messages = await sequelize.query(
            `SELECT (users.id = '${req.params.userId}') as issender, messages.userid, users.firstname, users.lastname, messages.body, messages.sendingtime AS stime, messages.viewtype ` +
            'FROM messages ' +
            'LEFT JOIN users ON users.id = messages.userid ' +
